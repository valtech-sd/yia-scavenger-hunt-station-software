...
class AuthenticationHelper {
   ...
   * @param res
   */
  static async authenticateOrThrow(req, res) {
    ...
    switch (securitySchemeDetails.type) {
      case 'basic':
        throw new HttpError(
          501,
          'Authentication type "basic" is not implemented.'
        );
        break;
      case 'apiKey':
        const requestApiKey = this.getSecurityApiKey(
          req,
          securitySchemeDetails.in,
          securitySchemeDetails.name
        );
        const hasValidRequestSecurity = await this.isValidApiKey(
          requestApiKey,
          req
        );
        if (!hasValidRequestSecurity)
          throw new HttpError(401, 'Not Authorized');
        break;
      case 'oauth2':
        throw new HttpError(
          501,
          'Authentication type "oauth2" is not implemented.'
        );
        break;
      default:
        throw new HttpError(
          501,
          `Authentication type "${securitySchemeDetails.type}" is not supported by the Swagger 2 specification.`
        );
    }
  }

  /**
   * Verify an API Key and decide if it's valid. Note, this is async in case the verification requires some
   * promise/asynch operation like a database read, etc.
   *
   * @param requestApiKey
   * @param req - - the request so that our authenticator can update it (for instance to add user level details)
   * @returns boolean - true for valid key or false for invalid key
   */
  static async isValidApiKey(requestApiKey, req) {
    // TODO: Implement better key authentication than a static string
    // Set the user object into the Express request object so that other handlers can access the user's info.
    // This is only needed if the application needs to differentiate user levels, groups, etc.
    req.user = {
      userAccess: 1,
    };
    // Return our bool indicating the apikey validity
  ...
  static getSecuritySchemeDetails(req) {
    ...
    const requestFirstSecuritySchemeName = this.getFirstRequestSecuritySchemeName(
      req
    );
    ...
    if (requestFirstSecuritySchemeName) {
      ...
      return req.swagger.api['securityDefinitions'][
        requestFirstSecuritySchemeName
      ];
    } else {
  ...
  static getFirstRequestSecuritySchemeName(req) {
    ...
    try {
      ...
      if (securityObject.length >= 1) {
        ...
        firstSecuritySchemeName = Object.keys(
          firstSecurityScheme
        )[0].toString();
      }
    } catch (ex) {
      ...
      console.log(
        `getFirstRequestSecuritySchemeName was not able to get the security scheme for the request to ${req.swagger.pathName}`
      );
    }
