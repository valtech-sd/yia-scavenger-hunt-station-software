...
global.logger.info(
  `Startup BOX_ID=${INTERNAL_BOX_ID}, SEQUENCE_ID=${INTERNAL_SEQUENCE_ID}`
);
...
/**
 * Validate the Quest Config Early and Fail if needed. This will THROW if the config is invalid
 * per a JSON SCHEMA check.
 */
...
// Wire in our CacheControl middleware and accept defaults
...
middleware.init(swaggerFilePath, (err) => {
  ...
  app.use(async (req, res, next) => {
    try {
      // Our authenticator is expected to return an http-error
      await AuthenticationHelper.authenticateOrThrow(req, res);
      next();
    } catch (httpError) {
      // Oppsie... let express handle the error
      next(httpError);
    }
  });
  ...
  try {
    ...
    throw new Error(
      `Unable to enumerate the controllers in the controllers directory. If a controller has a syntax or logic error (outside of methods) it will throw errors and cause this! The inner message was "${err.message}"`
    );
  }
  ...
  try {
    ...
    // NOTE: inside any controller, to throw an error, call `next(error)` and Express will handle it..
    ...
    throw new Error(
      'Unable to match Swagger x-swagger-router-controller & operationId to the enumerated controllers in the project. Double check to ensure you have identified all controllers with their proper name in your Swagger definition and that you have placed all the controllers in the proper directory.'
    );
  }
